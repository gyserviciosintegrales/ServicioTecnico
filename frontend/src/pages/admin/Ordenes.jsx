...
import { ClipboardList, Plus, Search, Edit2, Trash2, FileText, Clock, ArrowRight, User, ChevronDown, ChevronUp } from 'lucide-react';
              ...
              return (
                ...
                >
                  ...
                  <div style={{
                    ...
                    fontSize: '11px', zIndex: 1, marginTop: '2px',
                  }}>
                    {icon}
                  </div>
                    ...
                    {entrada.valor_anterior !== null && (
                      <div style={{
                        ...
                        <span style={{
                          ...
                          background: `${color}12`, border: `1px solid ${color}25`,
                          color, padding: '2px 7px', borderRadius: '5px',
                        }}>
                          {entrada.valor_nuevo || '—'}
                        </span>
                      </div>
...
export default function Ordenes() {
  ...
  const [historialAbierto, setHistorialAbierto] = useState(null); // id de la orden con historial abierto
  ...
  const save = async (e) => {
    ...
    try {
      ...
      // Si el historial estaba abierto para esta orden, cerrarlo y reabrir para refrescar
  ...
  const downloadPdf = async (id) => {
    ...
    try {
      ...
      a.href    = url;
      a.download = `orden_${String(id).padStart(4, '0')}.pdf`;
      a.click();
  ...
  const toggleHistorial = (id) => {
    setHistorialAbierto(prev => prev === id ? null : id);
  };
  ...
  return (
    <div style={{ animation: 'fadeInUp 0.4s ease' }}>
      ...
      {loading ? (
        ...
        <div className="card">
          <EmptyState icon={ClipboardList} title="Sin órdenes" subtitle="Creá la primera orden de trabajo" />
        </div>
      ) : (
                  ...
                  return (
                    <>
                      ...
                      <tr
                        key={`row-${o.id}`}
                        style={{
                          borderBottom: histAbierto ? 'none' : '1px solid var(--border)',
                          transition: 'background 0.15s',
                          background: histAbierto ? 'var(--bg-hover)' : 'transparent',
                        }}
                      ...
                      >
                        ...
                        <td style={{ padding: '12px 14px', fontSize: '12px', color: 'var(--text-secondary)' }}>
                          {o.tecnico_detalle?.nombre_completo
                            ? o.tecnico_detalle.nombre_completo
                        ...
                        <td style={{ padding: '12px 14px' }}>
                          <span style={{
                            ...
                            fontWeight: '600',
                            background: o.pagado ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)',
                            color:      o.pagado ? '#10b981' : '#ef4444',
                            border: `1px solid ${o.pagado ? 'rgba(16,185,129,0.25)' : 'rgba(239,68,68,0.25)'}`,
                          }}>
                        ...
                        <td style={{ padding: '12px 14px' }}>
                          <div style={{ display: 'flex', gap: '5px', justifyContent: 'flex-end', alignItems: 'center' }}>
                            ...
                            {/* Eliminar */}
                            <button
                              onClick={() => setConfirm({ open: true, id: o.id })}
                            ...
                            {/* ── Historial toggle ── */}
                            <button
                              onClick={() => toggleHistorial(o.id)}
                              title={histAbierto ? 'Cerrar historial' : 'Ver historial'}
                              style={{
                                ...btnBase,
                                borderColor:     histAbierto ? 'rgba(6,182,212,0.4)' : 'var(--border)',
                                color:           histAbierto ? '#06b6d4' : 'var(--text-secondary)',
                                background:      histAbierto ? 'rgba(6,182,212,0.08)' : 'var(--bg-hover)',
      ...
      <Modal
        ...
        title={modal.data
          ? `Editar Orden #${String(modal.data?.id || 0).padStart(4, '0')}`
          ...
          <div style={{ display: 'grid', gap: '14px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                ...
                >
                  ...
                  {equipos.map(eq => (
                    <option key={eq.id} value={eq.id}>
                      {eq.cliente_nombre} — {eq.tipo_display} {eq.marca} {eq.modelo}
                    </option>
                  ))}
                </select>
              </div>
              <div>
                ...
                >
                  ...
                  {tecnicos.map(t => (
                    <option key={t.id} value={t.id}>{t.nombre_completo} — #{t.legajo}</option>
                  ))}
                </select>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Descripción del problema que trae el cliente..."
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '12px' }}>
              ...
              <div>
                ...
                <input
                  type="number" className="input-field" min="0" max="24"
                  value={form.meses_garantia}
                  onChange={e => setForm({ ...form, meses_garantia: e.target.value })}
                />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
              <div>
                ...
                <input
                  type="number" className="input-field" min="0" step="0.01"
                  value={form.importe_mano_obra}
                  onChange={e => setForm({ ...form, importe_mano_obra: e.target.value })}
                />
              </div>
              <div>
                ...
                <input
                  type="number" className="input-field" min="0" step="0.01"
                  value={form.importe_repuestos}
                  onChange={e => setForm({ ...form, importe_repuestos: e.target.value })}
                />
              </div>
              <div>
                ...
                <div className="input-field" style={{
                  ...
                  ${(parseFloat(form.importe_mano_obra || 0) + parseFloat(form.importe_repuestos || 0))
      ...
      <ConfirmDialog
        ...
        loading={saving}
        title="¿Eliminar orden?"
        message="Se eliminará permanentemente esta orden de trabajo."
      />
    </div>
  );
}